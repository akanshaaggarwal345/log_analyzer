#Here we are going to parse our log files and save them in a csv file named as log_contents.csv.The log file we have is an apache logfile which is using combined log format 
...
# So the log is going to look like :199.30.24.222 - - [31/May/2017:01:17:49 +0530] "GET /images/img2.jpg HTTP/1.1" 200 16276 "-" "Mozilla/5.0 (iPhone; CPU iPhone OS 7_0 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11A465 Safari/9537.53 BingPreview/1.0b"
import glob 
import re

file2=open("log_contents.csv","w") # creating the new file
#defining splitcount functions which is going to split  the contents of log file
def splitcontent(logfiles):
	...
	for line in file1:
		list1=line.split("]")[0].split("[")[0].split(" ") # split till date first and then split based on " " and store the contents in a list
		del(list1[3])
    
		ipaddress=list1[0] # extract the useful information from the list
		uid=list1[1]
		username=list1[2]
